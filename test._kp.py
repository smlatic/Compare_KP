...
def read_reference_csv(file_path):
    ...
    df.columns = [col.upper() for col in df.columns]  # Convert columns to uppercase
    pipelines = {}
...
def compare_rows(row1, row2, columns, tolerance=0.001):
    discrepancies = []
    for col in columns:
        try:
            val1 = float(row1[col])
            ...
            if abs(val1 - val2) > tolerance:
                discrepancies.append(f"  Column '{col}': File value = {val1}, Reference value = {val2}")
        except ValueError:
...
def cross_check(data_from_files, reference_data):
    ...
    for ID, file_data in data_from_files.items():
        if ID in reference_data:
            ...
            # Compare end points
            end_discrepancies = compare_rows(
                file_data['end'],
                ref_data['end'],
                columns_to_compare
            )
